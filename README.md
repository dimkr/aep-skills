...
The AEP spec the skills follow ships in `plugins/aep-skills/reference/tootik-client.md` and is referenced from the skills via `${CLAUDE_PLUGIN_ROOT}`, so it travels and versions with the plugin. The `tootik-client` wrapper ships in `plugins/aep-skills/bin/` and is added to the Bash tool's `PATH` automatically while the plugin is enabled — it is a small POSIX `sh` script that runs tootik's `cmd/tootik-client` (from the `shell` branch of `github.com/dimkr/tootik`) via `CGO_ENABLED=0 go run`, so no binary is committed and it works on both Linux and macOS.

The wrapper connects to a tootik instance identified by a **host**, remembered globally in `${CLAUDE_CONFIG_DIR:-~/.claude}/aep-skills/host`. It injects `-host` into every call; on the first run with no host configured it prints instructions to ask the user for the host and save it (the host is never guessed). A `SessionStart` hook pre-warms the `go run` build so the first real call isn't slow.
...
- `go` on `PATH` and network access to the Go module proxy / `github.com` — the bundled `tootik-client` wrapper uses `go run` to build and run `cmd/tootik-client` on demand (the first run compiles it; Go caches the build afterward).
...
- The `bin/tootik-client` wrapper avoids hardcoded macOS paths and defers to `CGO_ENABLED=0 go run cmd/tootik-client@shell`, so it works on both Linux and macOS with no per-OS build step.
